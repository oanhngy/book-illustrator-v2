...
Mục tiêu **không phải** là làm cho xong, mà là **hiểu trọn vẹn từng dòng code trong dự án**.

- **Không có deadline.** Chậm mà hiểu tốt hơn nhanh mà xong.
...
Storage bằng JSON file là lựa chọn **có chủ ý và khó hơn** database: không transaction, không `UPDATE ... WHERE`, phải tự lo concurrency. Đây chính là phần tôi muốn học.
...
**Chưa có dòng code nào.** Chưa có `server/`, chưa có `client/`, chưa có tài liệu đặc tả.

Điều này quan trọng với Claude: **đừng giả định file nào tồn tại.** Trước khi nhắc tới một file, hãy kiểm tra nó có thật không. Tài liệu đặc tả (requirements, architecture, data model, API contract, testing...) sẽ được **tôi viết dần theo từng phase trong `PLAN.md`**, không phải tạo sẵn hàng loạt ngay bây giờ.
...
- Viện lý do "cho nhanh" — dự án này không có ràng buộc thời gian.
...
3. Nếu task có logic → có test (hoặc có ghi rõ lý do cố tình không test).
